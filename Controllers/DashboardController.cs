...
namespace TaskManager.Controllers
{
    public class DashboardController : Controller
    {
        ...
        public ActionResult Tasks(int? projectID)
        {
            ...
            bool isOwner, isParticipant;

            string sqlCmd = String.Format("SELECT * FROM projects WHERE id = {0} AND userFK = {1}", projectID.ToString(), Session["ID"].ToString());
            SqlCommand ownerCheck = new SqlCommand(sqlCmd, connection);
            if (ownerCheck.ExecuteReader().HasRows)
                isOwner = true;
            else
            ...
            sqlCmd = String.Format("SELECT * FROM project_user_join WHERE userFK = {0} and projectFK = {1}", Session["ID"].ToString(), projectID.ToString());
            ...
            SqlDataReader sdr = cmd.ExecuteReader();
            while (sdr.Read())
                projectTasks.Add(new Task(sdr.GetInt32(2), sdr.GetString(3), sdr.GetString(4), sdr.GetString(5), sdr.GetString(6), sdr.GetInt32(7), sdr.GetInt32(8), sdr.GetString(9), sdr.GetString(10)));
            connection.Close();
        ...
        public ActionResult AddTask()
        {
            ...
            while(sdr.Read())
                users.Add(new Models.User(sdr.GetInt32(0), sdr.GetString(1), sdr.GetString(2)));
